...
_ALLOW_DESTRUCTIVE_CLIENTS_MIGRATION = os.getenv("ALLOW_DESTRUCTIVE_CLIENTS_MIGRATION", "false").strip().lower() == "true"
    ...
    with engine.begin() as connection:
        ...
        if "clientes" in table_names:
            ...
            if "comercio_id" not in columns:
                if not _ALLOW_DESTRUCTIVE_CLIENTS_MIGRATION:
                    raise RuntimeError(
                        "La tabla 'clientes' tiene un esquema legado sin 'comercio_id'. "
                        "Para evitar perdida de datos, se bloqueo la migracion destructiva automatica. "
                        "Haz backup y establece ALLOW_DESTRUCTIVE_CLIENTS_MIGRATION=true solo para ejecutar esta migracion de forma controlada."
                    )
        ...
        if "admin_usuarios" in table_names:
            ...
            admins = connection.execute(text("SELECT id, username, password FROM admin_usuarios")).mappings().all()
            for admin in admins:
